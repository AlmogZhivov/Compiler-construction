...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec list_to_scheme_list = function
    | [] -> ScmNil
    | car :: cdr -> ScmPair(car, list_to_scheme_list cdr)

  let rec zip_with func list1 list2 =
    ...
    let rec macro_expand_let_star = function
      | ScmPair(ScmSymbol "let*", ScmPair(ScmNil, body)) ->
          ScmPair(ScmSymbol "let", ScmPair(ScmNil, body))  
      | ScmPair(ScmSymbol "let*", ScmPair(ScmPair(first, ScmNil), body)) -> 
          ScmPair(ScmSymbol "let", ScmPair(ScmPair(first, ScmNil),body))
      | ScmPair(ScmSymbol "let*", ScmPair(ScmPair(ScmPair(var, value), rest), body)) ->
          ScmPair(ScmSymbol "let",
                  ScmPair(ScmPair(ScmPair(var, value), ScmNil),
                          ScmPair(ScmPair(ScmSymbol "let*", ScmPair(rest, body)), ScmNil)))
      | _ -> raise (X_syntax "Malformed let*-expression!")

    let rec split_ribs = function
        | ScmNil -> ([],[])
        | ScmPair (ScmPair (var, ScmPair (value, scmNil)), ribs ) -> 
          let (vars, vals) = split_ribs ribs in 
          (var :: vars, value:: vals)
        | _ -> raise (X_syntax "Invalid let-rec rib structure");; 
  

  let help_whatever name ribs =
    ScmPair(
      ScmPair(name,
              ScmPair(
                ScmPair(ScmSymbol "quote",
  ...
  | ScmPair(ScmSymbol "letrec", ScmPair(ScmNil, body)) ->
      (* No bindings *)
      ScmPair(ScmSymbol "let", ScmPair(ScmNil, ScmPair(ScmPair(ScmSymbol "begin", body), ScmNil)))
  | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
    let (names, values) = split_ribs ribs in 
    let initial_bindings = List.fold_right(fun name acc -> help_whatever name acc) names ScmNil in
    let sets = List.map2 (fun name value -> ScmPair (ScmSymbol "set!", ScmPair (name, ScmPair (value, ScmNil)))) names values in
    let combined_body = 
      ScmPair (ScmSymbol "begin", 
               List.fold_right (fun car cdr -> ScmPair (car, cdr)) sets exprs) in
    ScmPair(ScmSymbol "let", ScmPair(initial_bindings, ScmPair(combined_body, ScmNil)))
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair(ScmSymbol "if",
          ScmPair(test,
            ScmPair(dit,
              ScmPair(dif,ScmNil)))) -> ScmIf(tag_parse test,
                                               tag_parse dit,
                                                tag_parse dif)
    ...
    | ScmPair (ScmSymbol "define", ScmPair (ScmPair (var, params), exprs)) -> 
      ...
      let define_expr = ScmPair (var, ScmPair (lambda_expr, ScmNil)) in
      tag_parse (ScmPair (ScmSymbol "define", define_expr))
    ...
    | ScmPair(ScmSymbol "let", ScmPair(ScmNil, ScmPair(body, ScmNil))) ->
      let lambda_expr = tag_parse (ScmPair(ScmSymbol "lambda", ScmPair(ScmNil, ScmPair(body, ScmNil)))) in
      ScmApplic (lambda_expr, [])
    (* with ribs*)
    
    | ScmPair (ScmSymbol "let" ,ScmPair (ribs, exprs)) ->
      let (vars, vals)= split_ribs ribs in
      let params = list_to_scheme_list vars in
      let args = list_to_scheme_list vals in
      tag_parse 
    ...
    | ScmPair(ScmSymbol "let*", rest) ->
        tag_parse (macro_expand_let_star (ScmPair(ScmSymbol "let*", rest)))
  
    (* support for letrec *)
    | ScmPair (ScmSymbol "letrec", rest) ->
      let expanded = macro_expand_letrec (ScmPair(ScmSymbol "letrec", rest)) in
      tag_parse expanded
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      (* support for if *)
      | ScmIf' (test, dit, dif) ->
         ScmIf' (run false test, (* the test is never in tail position *)
                 run in_tail dit,
                 run in_tail dif)
      (* support for sequences *)
      | ScmSeq' [] -> ScmSeq' []
      | ScmSeq' (expr :: exprs) -> 
        ScmSeq' (runl in_tail expr exprs)  (* First expr is not in tail position *)    
      (* support for or *)
      | ScmOr' [] -> ScmOr' []
      | ScmOr' (expr :: exprs) -> 
        ScmOr' (runl in_tail expr exprs)  (* First expr is not in tail position *)    
      ...
      | ScmLambda' (params, kind, expr) ->
         ScmLambda' (params, kind, run true expr) (* The entire body of lambda is in tail position *)
      (* support for applic *)
      (*| ScmApplic' (proc, args, app_kind) ->
         if in_tail
         then ScmApplic' (run false proc, List.map (run false) args, Tail_Call) (* only the last call is in tail position *)
         else ScmApplic' (run false proc, List.map (run false) args, Non_Tail_Call)*)
         (* TODO *)
      | ScmApplic'(proc, args, app_kind) -> 
          ScmApplic'(run false proc, (List.map (fun arg -> run false arg) args),
          (if in_tail 
            then Tail_Call 
          else Non_Tail_Call))    
      and runl in_tail expr = function
      | [] -> [run in_tail expr] (* here we reach the last expr and it's in tail position *)
      | expr' :: exprs -> 
          (run false expr) :: (runl in_tail expr' exprs)
...
module Code_Generation (* : CODE_GENERATION *) = struct
  ...
  let collect_constants =
    let rec run = function
      ...
      (* the case if varset is free we add it *)
      | ScmVarSet' (Var' (var, Free), expr) -> [ScmString var] @ (run expr)
      | ScmVarSet' (_, expr) -> run expr
      (* the case if vardef is free we add it *)
      | ScmVarDef' (Var' (var, Free), expr) -> [ScmString var] @ (run expr)
      | ScmVarDef' (_, expr) -> run expr
      (* the case if scmbox is free we add it *)
      | ScmBox' (Var' (var, Free)) -> [ScmString var]
      | ScmBox' _ -> []
      (* the case if scmboxget is free we add it *)
      | ScmBoxGet' (Var' (var, Free)) -> [ScmString var]
      | ScmBoxGet' _ -> []
      (* the case if scmboxset is free we add it *)
      | ScmBoxSet' (Var' (var, Free), expr) -> [ScmString var] @ (run expr)
  ...
  let const_repr sexpr loc table = match sexpr with
    ...
    | ScmNumber (ScmFraction (numerator, denominator)) ->
        ([RTTI "T_fraction"; Quad numerator; Quad denominator], 1 + word_size + word_size)
    | ScmNumber (ScmReal x) ->
    ...
    | ScmVector s -> 
        let addrs = List.map
                      (fun sexpr -> ConstPtr (search_constant_address sexpr table))
                      s in
    ...
    | ScmPair (car, cdr) ->
      (* Pairs are tagged with the T_PAIR type. The data of the first qword after the tag is the car pointer
        ...
        ([RTTI "T_pair"; ConstPtr car_addr; ConstPtr cdr_addr], 1 + word_size + word_size)
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmOr' exprs' ->
        let label_or_end = make_or_end () in
        let compiled_exprs = 
          List.map 
            (fun expr -> 
               (run params env expr) ^ 
      ...
      | ScmVarSet' (Var' (v, Free), expr') -> (*set free - seach in free var table*)
          let expr_code = run params env expr' in
          let label_var_in_FVAR = search_free_var_table v free_vars in
          expr_code ^ Printf.sprintf "\tmov qword [%s], rax\n" label_var_in_FVAR
          ^ "\tmov rax, sob_void\n"

      | ScmVarSet' (Var' (v, Param minor), ScmBox' _) -> (*set param box- on the stack- box?*)
        "\tmov rdi, 8\n"
        ^ "\tcall malloc\n" (* put a new address alocated for the box size 8 (array of size 1)*)
        ^ (Printf.sprintf "\tmov rbx, qword [rbp + 8 * (4 + %d)]\n" minor)
        ^ "\tmov qword[rax], rbx\n"
        ^ (Printf.sprintf "\tmov qword [rbp + 8 * (4 + %d)], rax\n" minor)
        ^ "\tmov rax, sob_void\n"

      | ScmVarSet' (Var' (v, Param minor), expr') -> (*set param - on the stack*)
        (run params env expr')
        ^ (Printf.sprintf "\tmov qword [rbp + 8 * (4 + %d)], rax\n" minor)
         ^ "\tmov rax, sob_void\n"

      | ScmVarSet' (Var' (v, Bound (major, minor)), expr') -> 
        (*set Bound - need to acces the stack major rib then minor rib and put it in place*)
        (run params env expr')
      ...
      | ScmLambda' (params', Simple, body) ->
         ...
         ^ "\tmov rsi, 0\n"
         ^ "\tmov rdx, 1\n"
         ^ (Printf.sprintf "%s:\t; ext_env[i + 1] <-- env[i] copy pointers from Env to ExtEnv\n"
              label_loop_env)
      ...
      | ScmLambda' (params', Opt opt, body) ->
         ...
         and label_loop_arity_exact = make_lambda_opt_loop ()
         and label_loop_arity_exact_exit = make_lambda_opt_loop_exit ()
         and label_loop_arity_more = make_lambda_opt_loop ()
         and label_loop_arity_more_exit = make_lambda_opt_loop_exit ()
         and label_loop_list_insertion = make_lambda_opt_loop ()
         and label_loop_list_insertion_end = make_lambda_opt_loop_exit ()
         in
         ...
         (* the case where we need to extend the stack *)
         ^ (Printf.sprintf "%s:\n" label_arity_exact) (* Start of arity exact logic *)
         ^ (Printf.sprintf "\tmov qword [rsp + 8 * 2], %d\t; Update argument count (fixed params + 1 for nil)\n" ((List.length params') + 1))
         ^ (Printf.sprintf "\tmov r10, %d\t; r10 = total addresses to adjust (fixed params + 3)\n" ((List.length params') + 3))
         ^ "\tpush qword [rsp]\t; Save the top stack element\n"
         ^ "\tmov r11, 1\t; r11 = loop index, starts at 1\n"
         
         ^ (Printf.sprintf "%s:\n" label_loop_arity_exact) (* Start of the adjustment loop *)
         ^ "\tcmp r11, r10\t; Compare loop index with total elements to adjust\n"
         ^ (Printf.sprintf "\tje %s\t; Exit loop if all elements processed\n" label_loop_arity_exact_exit)
         ...
         ^ (Printf.sprintf "\tjmp %s\t; Jump back to loop start\n" label_loop_arity_exact)
         
         ^ (Printf.sprintf "%s:\n" label_loop_arity_exact_exit) (* End of adjustment loop *)
         ^ "\tmov qword [r12], sob_nil\t; Place sob_nil in the freed slot to represent an empty optional argument list\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_stack_ok) (* End of arity exact logic *)
        
         (* the case of shrinking the stack *)
         ^ (Printf.sprintf "%s:\n" label_arity_more) (* Start of label_arity_more *)
         ^ "\tmov r8, qword [rsp + 8 * 2]\t; r8 = the total number of params in stack\n"
         ^ (Printf.sprintf "\tsub r8, %d\t; r8 = count of optional params\n" (List.length params'))
         ^ (Printf.sprintf "\tmov qword [rsp + 8 * 2], %d\t; Update the number of params (fixed params + 1 for list opt)\n" ((List.length params') + 1))
         ^ "\tmov r9, 0\t; r9 = index of optional argument list\n"
         ^ (Printf.sprintf "\tlea r10, [rsp + 2 * 8 + %d * 8 + r8 * 8]\t; r10 = address of first optional param\n" (List.length params'))
         
         (* optional list construction out of pairs *)
         ^ "\tmov r11, sob_nil\t; r11 = sob_nil, current pair in the linked list\n"
         ^ (Printf.sprintf "%s:\n" label_loop_arity_more) (* Start of stack_ok_loop_opt_label *)
         ^ "\tcmp r9, r8\t; Compare index with count of optional params\n"
         ^ (Printf.sprintf "\tje %s\t; Exit loop if all optional params are processed\n" label_loop_arity_more_exit)
         ...
         ^ "\tmov SOB_PAIR_CDR(rax), r11\t; Set CDR of the pair to the current list\n"
          ...
          (* when creating lists from right to left, the last pair/element must be constructed first *)
         ...
         ^ (Printf.sprintf "\tjmp %s\t; Repeat the loop for the next param\n" label_loop_arity_more)
         
         (* start of label_loop_arity_more_exit - putting the constructed list in the stack*)
         ^ (Printf.sprintf "%s:\n" label_loop_arity_more_exit) 
         ...
         ^ (Printf.sprintf "\tadd r12, %d\t; Skip fixed params and return address\n" (2 * 8 + (List.length params') * 8))
         ^ (Printf.sprintf "\tmov r13, %d\t; r13 = loop counter (number of params, old fp, ret addr, lex env)\n" ((List.length params') + 3))
         ^ (Printf.sprintf "%s:\n" label_loop_list_insertion) (* Start of label_loop_list_insertion *)
         ^ "\tcmp r13, 0\t; Check if there are more values to copy\n"
         ^ (Printf.sprintf "\tje %s\t; Exit loop if all values are copied\n" label_loop_list_insertion_end)
         ...
         ^ (Printf.sprintf "\tjmp %s\t; Repeat the loop for the next value\n" label_loop_list_insertion)
         ^ (Printf.sprintf "%s:\n" label_loop_list_insertion_end) (* End of label_loop_list_insertion *)
         ^ "\tadd r10, 8\t; Adjust r10 to point to the correct new stack top\n"
         ^ "\tmov rsp, r10\t; Update the stack pointer to the new top\n"
         ^ (Printf.sprintf "%s:\n" label_stack_ok) (* Stack handling complete *)
         
         ^ "\tenter 0, 0\t; Set up a new stack frame for the lambda body\n"
         ^ (run ((List.length params') + 1) (env + 1) body) (* Generate code for the lambda body *)
         ^ "\tleave\t; Restore the previous stack frame\n"
         ^ (Printf.sprintf "\tret AND_KILL_FRAME(%d)\n" (List.length params' + 1))
         ^ (Printf.sprintf "%s:\t; new closure is in rax\n" label_end) (* End of the lambda-opt handling *)
      ...
      | ScmApplic' (proc, args, Tail_Call) -> 
        let startLoop = make_tc_applic_recycle_frame_loop() in
...
(* end-of-input *)
 ...
 (*let test = Code_Generation.compile_scheme_file "init.scm" "testing/goo.asm";;*)
