...
let rec nt_expr str = make_nt_sub_add str
and make_nt_sub_add str = 
  let nt1 = pack (char '+') (fun _ -> Add) in
  let nt2 = pack (char '-') (fun _ -> Sub) in
  let nt1 = disj nt1 nt2 in
  (* hirarchy *)
  let nt1 = star (caten nt1 make_nt_mul_div_mod) in
  let nt1 = not_followed_by nt1 (unitify (make_nt_spaced_out (char '%'))) in
  let nt1 = pack (caten make_nt_mul_div_mod nt1)
  ...
  nt1 str
and make_nt_mul_div_mod str =
  let nt1 = pack (char '*') (fun _ -> Mul) in
  let nt2 = pack (char '/') (fun _ -> Div) in
  let nt3 = pack (word_ci "mod") (fun _ -> Mod) in 
  let nt1 = disj nt1 (disj nt2 nt3) in
  (* hirarchy *)
  let nt1 = star (caten nt1 make_nt_mul_per) in
  let nt1 = not_followed_by nt1 (unitify (make_nt_spaced_out (char '%'))) in
  let nt1 = pack (caten make_nt_mul_per nt1)
        (fun (first, rest) -> List.fold_left
        (fun e1 (op, first') -> BinOp (op, e1, first'))
        first
        rest) in
...
and make_nt_mul_per str =
  let nt1 = make_nt_sub_add_per in
  let nt2 = pack (char '*') (fun _ -> Mul) in
  let nt2 = caten nt2 (caten make_nt_sub_add_per (unitify (make_nt_spaced_out (char '%')))) in
  let nt1 = caten nt1 (star nt2) in
  let nt1 = pack nt1 
    (fun (e, op) ->
      List.fold_left
        (fun e -> function
        | Mul, (e', _) -> BinOp (PerOf, e, e')
        | _ -> raise X_no_match)
        e
        op) in
  let nt1 = make_nt_spaced_out nt1 in
  nt1 str
and make_nt_sub_add_per str =
  let nt1 = make_nt_pow in
  let nt2 = disj (pack (char '+') (fun _ -> Add)) (pack (char '-') (fun _ -> Sub)) in
  let nt2 = caten nt2 (caten make_nt_pow (unitify (make_nt_spaced_out (char '%')))) in
  let nt1 = caten nt1 (star nt2) in
  let nt1 = pack nt1 
      (fun (e, op) ->
        List.fold_left
        (fun e -> function
        | Add, (e', _) -> BinOp (AddPer, e, e')
        | Sub, (e', _) -> BinOp (SubPer, e, e')
        | _ -> raise X_no_match)
        e
        op) in
  let nt1 = make_nt_spaced_out nt1 in
...
and make_nt_pow str =
    let nt1 = pack (caten (char '^') make_nt_call_or_deref) (fun (_, e) -> e) in
    let nt1 = star nt1 in
    let nt1 = caten make_nt_call_or_deref nt1 in
...
let make_nt_div_unary_paren =
  make_nt_paren '(' ')' 
    (pack (caten (make_nt_spaced_out (char '/')) nt_expr) 
      (fun (_, e) -> BinOp (Div, Num 1, e))) in
let make_nt_sub_unary_paren =
  make_nt_paren '(' ')' 
    (pack (caten (make_nt_spaced_out (char '-')) nt_expr) 
      (fun (_, e) -> BinOp (Sub, Num 0, e))) in
  let nt1 = disj_list [nt_call_or_deref; make_nt_div_unary_paren; make_nt_sub_unary_paren; nt_number; nt_var; nt_paren] in
...
open InfixParser;;
let test_string (nt : 'a PC.parser) str index =
  nt str index;;
